...
func UpdateEmail(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Некорректный id пользователя для обновления email", http.StatusBadRequest)
	...
	if err != nil {
		fmt.Printf("ошибка поиска пользователя в БД: %v", err)
		http.Error(w, "Ошибка поиска пользователя в БД", http.StatusBadRequest)
		return
	}
	if usr == nil {
		fmt.Printf("ошибка поиска пользователя: %v", err)
	...
	err = service.ValidateUpdateEmail(id, emailData.Email, emailData.Password)
	if err != nil {
